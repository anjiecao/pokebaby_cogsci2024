...
limits on the computing power (i.e.~increasing dimension would lead to
exponential increase in the total run time)., The first three components
which captured 57.9\% of the variance. A visualization of experimental
...
embedding space. We assume this concept describes the distribution of
all viewed stimuli in this experiment. This concept \((\mu,\sigma)\)
generates exemplars \((y)\) of the concept. Each exemplar corresponds to
one stimulus observed. RANCH observes repeated noisy samples
...
included two levels: animacy (e.g.~animate stimuli such as the animals
or inanimate stimuli such as the vegetables), number (singleton or
pair), and pose (facing left or right).
...
randomly sampled from the stimuli pool without replacement. The deviant
...
of the term was significant (all \emph{p} \textgreater{} 0.05).
